/*
 * версия11 13 сентября 2013г.
 * 
 * Нужно сделать:
 * 1) а главном окне добавить возможность растяжения любого 1ого графика на все окно
 * 2) в окнах детального просомтра рисовать графики через cairo surface
 * 3) уменьшить кнопки сверх
 * 4) добавить кнопки величения, влево-вправо и тп на главное окно
 */
...
static void do_drawing(cairo_t *cr, gpointer user_data)
{
	...
	for(i=0; i<4; i++) {
		cairo_set_source_rgb(cr, 0.969, .969, .969);
		cairo_rectangle(cr, .0+220.0*i, .0, 170.0, 125.0);
		cairo_fill(cr);
		cairo_set_source_rgb(cr, 0.1, 0.1, 0.1);
		cairo_move_to(cr, 100.0+220*i, 30.0);
		cairo_show_text(cr, concat_str("D", i+1));
	}	
	
	for(i=0; i<6; i++) {
		cairo_set_source_rgb(cr, 0.969, .969, .969);
		cairo_rectangle(cr, 0+220.0*i, 130.0, 170.0, 125.0);
		cairo_fill(cr);
		cairo_set_source_rgb(cr, 0.1, 0.1, 0.1);
		cairo_move_to(cr, 100.0+220*i, 130.0+30.0);
		cairo_show_text(cr, concat_str("T", i*2+1));
	}
	for(i=0; i<6; i++) {
		cairo_set_source_rgb(cr, 0.969, .969, .969);
		cairo_set_source_rgb(cr, 0.969, .969, .969);
		cairo_rectangle(cr, 0+220.0*i, 260.0, 170.0, 125.0);
		cairo_fill(cr);
		cairo_set_source_rgb(cr, 0.1, 0.1, 0.1);
		cairo_move_to(cr, 100.0+220*i, 260.0+30.0);
		cairo_show_text(cr, concat_str("T", (i+1)*2));
	}
	...
	if(glob.flag==1) {
		for(i=0; i<4; i++) {
			...
			for(j=0; j<=MAX_CH-1; j+=4)
				cairo_mask_surface(cr, pt_surface(), (double)(0.039*j+i*220), (120.0-(double)spectras->energyspk[i][j]/co_e));		
		}
		cairo_set_source_rgb(cr, 1.0, 0.0, 0.0);
		int max_t = max_time();
		for(i=0; i<6; i++) {
			double co_t = (double)(max_t/120.0)+1.0;
			for(j=0; j<=MAX_CH-1; j+=4)
				cairo_mask_surface(cr, pt_surface(), (double)(0.039*j+i*220), (250.0-(double)spectras->timespk[i*2][j]/co_t));
		}
		for(i=0; i<6; i++) {
			double co_t = (double)(max_t/120.0)+1.0;
			for(j=0; j<=MAX_CH-1; j+=4)
				cairo_mask_surface(cr, pt_surface(), (double)(0.039*j+i*220), (380.0-(double)spectras->timespk[i*2+1][j]/co_t));
		}
	}
}
...
{	
	...
	darea = gtk_drawing_area_new();
	gtk_widget_set_size_request (darea, 1000, 400);
	g_signal_connect(G_OBJECT(darea), "draw", 
