...
void draw_func_energy(cairo_t *cr, int num, char *str)
{
	...
	if(glob.flag==1) {
		...
		if(mainflag.maximize == 0) {
			...
			if(mainflag.zoom_en == -1) {mainflag.zoom_en = 0; x_start = 0;}
			if(mainflag.zoom_eny == -1) {mainflag.zoom_eny = 0; y_comp = 1.0;}
			x_start += mainflag.zoom_en*ZOOM_EN_X;
			y_comp *= 1.0+mainflag.zoom_eny*ZOOM_EN_Y;
			if(mainflag.zoom_en == 1) mainflag.zoom_en = 0;
			if(mainflag.zoom_eny == 1) mainflag.zoom_eny = 0;
			...
			if(mainflag.log_en != 1)
				for(j=x_start; j<=MAX_CH-1-x_start; j+=2)
					cairo_mask_surface(cr, pt_surface(), (double)(6.0*X_SIZE_PREPLOT/(MAX_CH-2.0*x_start)*(j-x_start-x_left+x_right)), (2.0*Y_SIZE_PREPLOT-(double)spectras->energyspk[num][j]/y_comp));
			else
				for(j=x_start; j<=MAX_CH-1-x_start; j+=2)
					cairo_mask_surface(cr, pt_surface(), (double)(6.0*X_SIZE_PREPLOT/(MAX_CH-2.0*x_start)*(j-x_start-x_left+x_right)), (2.0*Y_SIZE_PREPLOT-10.0*log( (double)spectras->energyspk[num][j]/y_comp )));
		
			cairo_set_source_rgb(cr, 0.0, 0.0, 0.0); 
			cairo_set_line_width(cr, 1.0);
			cairo_move_to(cr, 0.0, 2.0*Y_SIZE_PREPLOT);
			cairo_line_to(cr, 6.0*X_SIZE_PREPLOT, 2.0*Y_SIZE_PREPLOT);
						cairo_stroke(cr);
			if(mainflag.log_en != 1)
				sprintf(coord[0], "%.2fK; %.2fK", (double)(x_start+x_left-x_right)/1000.0, (Y_SIZE_PREPLOT*y_comp)/1000.0);
			else
				sprintf(coord[0], "%.2fK; exp(%.1f) ln", (double)(x_start+x_left-x_right)/1000.0, 2.0*Y_SIZE_PREPLOT);
			cairo_show_text(cr, coord[0]);
			...
			for(i=0; i<=5; i++) {
				tics[i] = 25*((int)(x_start)/25+i+1);
				sprintf(coord[i], "%d", tics[i]);
				cairo_move_to(cr, tics[i]*6.0*X_SIZE_PREPLOT/4096.0, 2.0*Y_SIZE_PREPLOT-10);
				cairo_show_text(cr, coord[i]);
			}
			}
			else if(MAX_CH-2.0*x_start < 200) {
			for(i=0; i<=5; i++) {
				tics[i] = 50*((int)(x_start)/50+i+1);
				sprintf(coord[i], "%d", tics[i]);
				cairo_move_to(cr, tics[i]*6.0*X_SIZE_PREPLOT/4096.0, 2.0*Y_SIZE_PREPLOT-5);
				cairo_show_text(cr, coord[i]);
			}
			}
			else if(MAX_CH-2.0*x_start < 500) {
			for(i=0; i<=5; i++) {
				tics[i] = 100*((int)(x_start)/100+i+1);
				sprintf(coord[i], "%d", tics[i]);
				cairo_move_to(cr, tics[i]*6.0*X_SIZE_PREPLOT/4096.0, 2.0*Y_SIZE_PREPLOT-5);
				cairo_show_text(cr, coord[i]);
			}
			...
			for(i=0; i<=5; i++) {
				tics[i] = 250*((int)(x_start/250.0)+i+1);
				sprintf(coord[i], "%d", tics[i]);
				cairo_move_to(cr, tics[i]*6.0*X_SIZE_PREPLOT/4096.0, 2.0*Y_SIZE_PREPLOT-5);
				cairo_show_text(cr, coord[i]);
			}
			...
			for(i=0; i<=4; i++) {
				tics[i] = 500*((int)(x_start/500.0)+i+1);
				sprintf(coord[i], "%d", tics[i]);
				cairo_move_to(cr, tics[i]*6.0*X_SIZE_PREPLOT/4096.0, 2.0*Y_SIZE_PREPLOT-5);
				cairo_show_text(cr, coord[i]);
			}
			}
			else if(MAX_CH-2.0*x_start < 4000) {
			for(i=0; i<=3; i++) {
				tics[i] = 1000*((int)(x_start/1000.0)+i+1);
				sprintf(coord[i], "%d", tics[i]);
				cairo_move_to(cr, tics[i]*6.0*X_SIZE_PREPLOT/4096.0, 2.0*Y_SIZE_PREPLOT-5);
				cairo_show_text(cr, coord[i]);
			}
               ...
               gpointer   data )
{
	...
	mainflag.zoom_en = -1; mainflag.zoom_eny = -1; mainflag.log_en = -1; mainflag.left_en = -1; mainflag.right_en = -1;
...
cairo_surface_t *pt_surface()
{
	...
	cairo_set_line_width (context, 0.0001);
	cairo_arc(context, 3, 3, 0.5, 0, 2.0*M_PI);
...
/*	for(i=0; i<=11; i++) {
		...
		strcat(name, ItoA);
		strcat(name, ".SPK");
		sprintf(name, "%s%s%d%s". folder2spk, "TIME", i+1, ".SPK");
		fd_out[i] = open(name, O_WRONLY|O_CREAT, S_IRUSR|S_IWUSR);
	...
	for(i=12; i<=15; i++) {
		...
		strcat(name, ItoA);
		strcat(name, ".SPK");
		sprintf(name, "%s%s%d%s". folder2spk, "BUFKA", i-11, ".SPK");
		fd_out[i] = open(name, O_WRONLY|O_CREAT, S_IRUSR|S_IWUSR);
...
{	
    ...
    GtkToolItem *button, *button_get_data, *button_read, *separator1, *button_energy, *button_time;
   // GtkWidget *button_energy, *button_time;
    GtkWidget *hseparator;
    GtkWidget *grid_control;
    GtkWidget *scrolled_win;
    GtkWidget *button_mini, *image_mini;
    GtkWidget *en_label, *button_en_left, *img_en_left, *button_en_log, *button_en_right, *img_en_right, *button_en_zoom_in, *img_en_zoom_in, *button_en_zoom_in_y, *img_en_zoom_in_y;
